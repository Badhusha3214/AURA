<template>
  ...
  <div class="calendar-container bg-gradient-to-b from-white via-pink-100 to-pink-100 text-gray-700 rounded-lg shadow-md p-8  mb-20">
    ...
    <div class="calendar">
      <div v-for="month in months" :key="month.name" class="month-section">
        <div class="month-header flex justify-between items-center mb-2">
          <div class="month cursor-pointer px-4 py-2 rounded-md text-pink-500 hover:bg-pink-500 hover:text-white"
        ...
        <div v-if="showNotes[month.name]" class="notes bg-white p-4 rounded-md shadow-md mt-4">
...
export default {
  ...
  methods: {
    ...
    toggleNotes(monthName) {
      this.showNotes[monthName] = !this.showNotes[monthName];
    },
    markDate(date) {
